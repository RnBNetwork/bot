...
async function connectMikrotik(serverKey) {
    ...
    const api = new RouterOSAPI({
        host: targetServer.mikrotik.host, port: targetServer.mikrotik.port,
        user: targetServer.mikrotik.user, password: targetServer.mikrotik.pass, timeout: 15
    });
    try { await api.connect(); return { api, targetServer }; } 
    catch (err) { throw new Error(`Gagal konek MikroTik ${targetServer.label}. Cek port API.`); }
}
...
client.on('message_create', async (msg) => {
    try {
        ...
        if (['!cek', '!aktifkan'].includes(command)) {
            if (args.length < 3) {
                // ✅ Backtick sudah ditutup dengan benar di sini
...
async function handleCekRedaman(msg, serverKey, username) {
    ...
    try {
        ...
        if (!rawMac || rawMac === 'Any') { await msg.reply(`⚠️ *MAC Address tidak terbaca*\n\nUser "${username}" ditemukan, tetapi MAC address tidak tersedia.`); return; }
    ...
    } catch (err) { await msg.reply(`❌ *Gagal Cek Redaman*\n\n${err.message}`); } 
    finally { try { if (api) await api.close(); } catch (e) {} }
}
